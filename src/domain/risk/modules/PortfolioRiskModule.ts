...
export class PortfolioRiskModule implements RiskModule {
  async evaluate(input: RiskEvaluationInput): Promise<RiskModuleResult> {
    ...
    const blockingRules = filterRulesByPrefix(legacyFacts.blocking_rules, ["portfolio."]);
    const warnings = filterWarningsByPrefix(legacyFacts.warnings, ["portfolio."]);
    const adjustments = filterAdjustmentsByPrefix(legacyFacts.adjustments, ["portfolio."]);

    return {
      module_name: "portfolio_risk",

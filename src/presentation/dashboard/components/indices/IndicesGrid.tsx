...
type GridFilter = "all" | "americas" | "europe" | "asia-pacific";
...
const REGION_LABELS: Record<IndexRegion, string> = {
  us: "Americas",
  europe: "Europe",
  asia: "Asia-Pacific",
};
...
function regionBadge(region: IndexRegion): string {
  if (region === "us") return "🇺🇸";
  if (region === "europe") return "🇪🇺";
  return "🌏";
}

function directionTone(direction: IndexSignalRow["direction"]) {
...
function matchesFilter(region: IndexRegion, filter: GridFilter): boolean {
  if (filter === "all") return true;
  if (filter === "americas") return region === "us";
  if (filter === "asia-pacific") return region === "asia";
...
export function IndicesGrid({
  ...
  const filteredPrices = prices.filter(price => matchesFilter(price.region, filter));
    ...
    <section className="space-y-5">
      <div className="mb-6 border-b border-[var(--apex-border-subtle)] pb-4">
        ...
        <p className="mt-2 text-[13px] text-[var(--apex-text-secondary)]">
          Global benchmarks, volatility proxies, and the latest APEX signal coverage.
        </p>
      </div>
          ...
          : filteredPrices.length === 0
            ? (
                <div className="col-span-full rounded-[var(--apex-radius-lg)] border border-[var(--apex-border-subtle)] bg-[var(--apex-bg-raised)] px-4 py-6 text-[13px] text-[var(--apex-text-secondary)]">
                  No index rows are available for this region yet.
                </div>
              )
            : filteredPrices.map(price => {
              ...
              return (
                <button
                  key={price.symbol}
                  type="button"
                  onClick={() => onSelectIndex(price.symbol)}
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="rounded-full border border-[var(--apex-border-subtle)] px-2 py-0.5 font-[var(--apex-font-mono)] text-[9px] uppercase tracking-[0.12em] text-[var(--apex-text-secondary)]">
                          {regionBadge(price.region)} {REGION_LABELS[price.region]}
                        </span>
                        <span className="rounded-full border border-[var(--apex-border-subtle)] px-2 py-0.5 font-[var(--apex-font-mono)] text-[9px] uppercase tracking-[0.12em] text-[var(--apex-text-secondary)]">
                          {providerBadge(price.provider)}
                        </span>
                        <span className={`rounded-full border px-2 py-0.5 font-[var(--apex-font-mono)] text-[9px] uppercase tracking-[0.12em] ${
                          price.marketStatus === "open"
                            ? "border-[rgba(80,160,100,0.35)] text-[var(--apex-status-active-text)]"
                            : "border-[var(--apex-border-subtle)] text-[var(--apex-text-secondary)]"
                        }`}>
                          {price.marketStatus}
                        </span>

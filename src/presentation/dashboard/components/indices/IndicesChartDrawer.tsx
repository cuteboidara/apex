...
function buildTradingViewUrl(symbol: string | null): string {
  const params = new URLSearchParams({
    symbol: symbol ? (TV_SYMBOL_MAP[symbol] ?? "SP:SPX") : "SP:SPX",
    interval: "60",
    theme: "dark",
    style: "1",
    timezone: "Etc/UTC",
...
export function IndicesChartDrawer({
  open,
  symbol,
  price,
  signal,
  ...
  open: boolean;
  symbol: string | null;
  price: IndexPriceRow | null;
  signal: IndexSignalRow | null;
  onClose: () => void;
}) {
  ...
  return (
    <div className={`fixed inset-0 z-50 ${open ? "pointer-events-auto" : "pointer-events-none"}`}>
      <button
        type="button"
        aria-label="Close index chart drawer"
        onClick={onClose}
            ...
            {signal?.grade ? (
              ...
              <div className="mt-5 rounded-[var(--apex-radius-md)] border border-[var(--apex-border-subtle)] bg-[rgba(255,255,255,0.02)] px-4 py-4">
                <p className="text-[13px] text-[var(--apex-text-secondary)]">
                  No active APEX signal is available for this index yet.
                </p>
              </div>
            )}
          </aside>

...
function isScalpStateArray(value: unknown): value is ScalpAssetState[] {
...
export function ScalpDashboard({ view = "all" }: { view?: DashboardView }) {
  ...
  const [assetStates, setAssetStates] = useState<ScalpAssetState[]>([]);
  ...
  async function fetchData() {
    try {
      const [signalsResponse, statesResponse] = await Promise.all([
        fetch("/api/scalp/signals", { cache: "no-store" }),
        fetch("/api/scalp/state", { cache: "no-store" }),
      ]);
      ...
      if (statesResponse.ok && isScalpStateArray(statePayload)) {
  ...
  async function runCycle() {
    ...
    try {
      ...
      const payload = await response.json().catch(() => null) as (ScalpCycleResponse & { error?: string }) | null;
      if (!response.ok) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 font-mono text-xs text-slate-500">
            5-Gate Confluence · 15m entries · 1-4h holds · Session: {stats?.currentSession ?? "-"} · Engine: {stats?.engine ?? "-"}
          </p>
        </div>
        <button
          type="button"
